...
export const readAlbumns: RequestHandler = async (req: Request, res: Response) => { 
  try { 
...
export const readAlbumsByAlbumDescriptionSearch: RequestHandler = async (req: Request, res: Response) => { 
  try{ 
    console.log('search', req.params.search); 
    const albums = await AlbumDao.readAlbumsByAlbumDescriptionSearch('%' + req.params.search + '%'); 
...
export const createAlbum: RequestHandler = async (req: Request, res: Response) => {
  try{ 
    ...
    req.body.tracks.forEach(async (track: Track, index: number) => {
      try {
        await TracksDao.createTracks(track, okPacket.insertId);
      } catch (error) {
        ...
        res.status(500).json({
          message: 'There was an error when writing to albums'
        }); 
      }
    };
      ...
      try{ 
        ...
        req.body.tracks.forEach(async (track: Track, index: number) => { 
          try{ 
            ...
            res.status(500).json({
              message: 'There was an error updating albums'
            }); 
          }
        });
        res.status(200).json(
